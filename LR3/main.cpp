...
int main(int argc, char** argv)
{
	...
	GLuint vaoAirShipBody;
	GLuint vaoAirShipConus;
	GLuint vaoAirShipCabin;
	GLuint vaoSphere;
	...
	GLsizei Sphere = CreateSphere(2.0f, 200, vaoSphere);
	


	GLsizei AirShipBody = CreateSphere(1.0f, 250, vaoAirShipBody);
	GLsizei AirShipConus = CreateCone(1.0f, 1.0f, 5000, vaoAirShipConus);


	GLuint vaoDer;
	GLsizei Der = CreateSphere(1.0f, 36, vaoDer);
	GLuint vaoKor;
	GLsizei Kor = CreateSphere(1.0f, 36, vaoKor);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float4x4 projection = projectionMatrixTransposed(camera.zoom, float(WIDTH) / float(HEIGHT), 0.1f, 999999999.0f);
		...
		program.SetUniform("pointLights[0].position", make_float3(xtimer, 10.0f,ytimer* ytimer));
		...
		program.SetUniform("pointLights[1].position", make_float3(0.0f, 28.0f, 0.0f));
		program.SetUniform("pointLights[1].color", make_float3(1.0f, 1.0f, 1.0f));
		program.SetUniform("pointLights[1].ambient", make_float3(0.6f, 0.6f, 0.6f));
		program.SetUniform("pointLights[1].specular", make_float3(1.0f, 1.0f, 1.0f));
		program.SetUniform("pointLights[1].mult", 50.4f);
		...
		program.SetUniform("material.shininess", 1.0f);

		program.SetUniform("typeOfTexture", false);



		glBindVertexArray(0); GL_CHECK_ERRORS		glBindVertexArray(vaoSquarePlane); GL_CHECK_ERRORS;
		{
		...
		glBindVertexArray(vaoSphere);
		{
			...
			model = transpose(mul(translate4x4(float3(0.0f,30.0f,0.0f)), scale4x4(float3(3.0f, 3.0f, 3.0f))));
			program.SetUniform("model", model); GL_CHECK_ERRORS;
		...
		glBindVertexArray(vaoTree2);
		{
			bindTexture(program, 0, "material.texture_diffuse", wood);
			for (int i = 0; i < 100; i++) {
				model = transpose(mul(translate4x4(float3(cos(20 * i) * 19.0f, -15.0f, cos(i) * 19.0f)), rotate_Z_4x4((90) * LiteMath::DEG_TO_RAD)));
				program.SetUniform("model", model); GL_CHECK_ERRORS;
				glDrawElements(GL_TRIANGLE_STRIP, Tree2, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			}
		...
		glBindVertexArray(vaoTree);
		{
			for (int i = 0; i < 100; i++) {
				bindTexture(program, 0, "material.texture_diffuse", tree);
				model = transpose(mul(translate4x4(float3(cos(20 * i) * 19.0f, -13.0f, cos(i) * 19.0f)), scale4x4(float3(0.5f, (0.7 + sin(i / 2) / 3 * pow((-1), i)) * 1.0f, 0.5f))));
				program.SetUniform("model", model); GL_CHECK_ERRORS;
				glDrawElements(GL_TRIANGLE_STRIP, Tree, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			}
		...
		///////////////////     Тело корабля
		glBindVertexArray(vaoAirShipBody);
		{
			bindTexture(program, 0, "material.texture_diffuse", shar);

			float4x4 modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(0.0f, 7.0f, -15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-10.0f, 15.0f, -15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-10.0f, -1.0f, -15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;


			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(0.0f, 7.0f, 15.0f)), scale4x4(float3(5.55f, 2.55f, 2.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipBody, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
		...
		glBindVertexArray(0); GL_CHECK_ERRORS;
		///////////////////    Конусы коробля
		glBindVertexArray(vaoAirShipConus);
		{
			bindTexture(program, 0, "material.texture_diffuse", shar);

			float4x4 modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(4.0f, 7.0f, -15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(4.0f, 6.25f, -15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-6.0f, 15.0f, -15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-6.0f, 14.25f, -15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-6.0f, -1.0f, -15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-6.0f, -2.25f, -15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-4.0f, 7.0f, 15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-4.0f, 6.25f, 15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			...
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-14.0f, 15.0f, 15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-14.0f, 14.25f, 15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;

			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-14.0f, -1.0f, 15.0f)), scale4x4(float3(1.55f, 3.55f, 0.10f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
			modelship = transpose(mul(rotate_Y_4x4(timer * 1.0), mul(translate4x4(float3(-14.0f, -2.25f, 15.0f)), scale4x4(float3(0.15f, 2.55f, 4.55f)))));
			program.SetUniform("model", modelship); GL_CHECK_ERRORS;
			glDrawElements(GL_TRIANGLE_STRIP, AirShipConus, GL_UNSIGNED_INT, nullptr); GL_CHECK_ERRORS;
